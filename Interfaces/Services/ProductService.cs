...
namespace goalongapi.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<(string errorMessage, string imageName)> UploadImage(
        ...
        public async Task<(string errorMessage, List<string> imageName)> UploadMultiFiles(
            List<IFormFile> formFiles
        )
        {
            string errorMesage = String.Empty;
        ...
        public async Task<(string errorMessage, List<string> imageName)> UploadMultiFilesReq(
            List<IFormFile> formFiles
        )
        {
            string errorMesage = String.Empty;
            ...
            if (uploadFileService.IsUpload(formFiles))
            {
                ...
                if (String.IsNullOrEmpty(errorMesage))
                {
                    imageName = await uploadFileService.UploadFileReq(formFiles);
                }
            }
            return (errorMesage, imageName);
        }
