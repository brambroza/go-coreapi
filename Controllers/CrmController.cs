...
namespace goalongapi.Controllers
{
    ...
    public class CrmController : ControllerBase
    {
        ...
        public IActionResult getreqfromcustlistNew([FromQuery] string userlogin, [FromQuery] string cmpid )
        {
            ...
            DataTable dtItem = DB.DBConn.GetDataTable($"exec dbo.[getReqFromCustomerItemId] @user='{userlogin}', @cmpid='{cmpid}'  ");
        ...
        [HttpGet("[action]")]
        public IActionResult getreqfromcustlistId([FromQuery] string userlogin, [FromQuery] string cmpid , [FromQuery] string ticketId)
        {
            // *** แนะนำ: culture ไม่จำเป็นถ้า format เป็น yyyy-MM-dd HH:mm
            // ใช้ InvariantCulture เร็วและชัดเจนกว่า
            var fmt = CultureInfo.InvariantCulture;
            ...
            // 1) Load DataTables (เหมือนเดิม)
            DataTable dt = DB.DBConn.GetDataTable($"exec dbo.[getReqFromCustomerId] @user='{userlogin}', @cmpid='{cmpid}' , @ticketId='{ticketId}' ");
            DataTable dtItem = DB.DBConn.GetDataTable($"exec dbo.[getReqFromCustomerItemId] @user='{userlogin}', @cmpid='{cmpid}' , @ticketId='{ticketId}' ");
            DataTable dtAssign = DB.DBConn.GetDataTable($"exec dbo.[getReqFromCustomerAssignId] @user='{userlogin}', @cmpid='{cmpid}' , @ticketId='{ticketId}' ");
            DataTable dtAssignDisplay = DB.DBConn.GetDataTable($"exec dbo.[getReqFromCustomerAssignDisplayId] @user='{userlogin}', @cmpid='{cmpid}' , @ticketId='{ticketId}'");
            DataTable dtOwner = DB.DBConn.GetDataTable($"exec dbo.[getReqFromCustomerOwnerId] @user='{userlogin}', @cmpid='{cmpid}' , @ticketId='{ticketId}'");
            DataTable dtRoute = DB.DBConn.GetDataTable($"exec dbo.[getReqFromCustomerRouteId] @user='{userlogin}', @cmpid='{cmpid}' , @ticketId='{ticketId}'");
